...
function generateTextPoints(text, count, fontSize = 200, fontFamily = 'Arial', xShift = -28) {
    ...
    // Increase sampling density for better resolution
    const step = 4;
    for (let i = 0; i < canvas.height; i += step) {
        for (let j = 0; j < canvas.width; j += step) {
            const index = (i * canvas.width + j) * 4;
            if (data[index + 3] > 128) {
                // Normalize to roughly -1 to 1 range based on canvas width/height ratio aspect
                // Canvas is 1024 width. 
                // x range: -25 to +25 roughly with 0.05 scale.
                const x = (j - canvas.width / 2) * 0.05;
                const y = (canvas.height / 2 - i) * 0.05;
                pixels.push(x, y);
            }
        }
    }
    ...
    for (let i = 0; i < count; i++) {
        // Use modulus to cycle through pixels if count > pixels.length to ensure we use all pixels
        // But random sampling is also fine if dense enough. 
        // Let's stick to random for now but formatted correctly.
        const pixelIndex = Math.floor(Math.random() * (pixels.length / 2)) * 2;
        points[i * 3] = pixels[pixelIndex] + xShift;
        points[i * 3 + 1] = pixels[pixelIndex + 1];
        points[i * 3 + 2] = 0;
    }
    return points;
}

function generateShapeWithText(shapeGenerator, name, totalCount, size) {
    ...
    // Use default xShift = -18 for existing shapes to maintain layout
    const textPoints = generateTextPoints(name, textCount);
...
const DATA = window.BRAINIT_DATA || {};

export const SHAPES = [
    {
        name: 'Sphere',
        title: DATA['Sphere']?.title || 'Consultora Brainit',
        description: DATA['Sphere']?.desc || 'La ventaja estratégica de tu empresa.',
        generator: (c, s) => generateShapeWithText(generateSphere, 'portfolio', c, s)
    },
    {
        name: 'Cube',
        title: DATA['Cube']?.title || 'Análisis de datos',
        description: DATA['Cube']?.desc || 'Análizamos y procesamos datos para estructurarlos.',
        generator: (c, s) => generateShapeWithText(generateCube, DATA['Cube']?.name || 'Datos', c, s)
    },
    {
        name: 'Pyramid',
        title: DATA['Pyramid']?.title || 'Datos Seguros',
        description: DATA['Pyramid']?.desc || 'Confiables, replicables y asegurados.',
        generator: (c, s) => generateShapeWithText(generatePyramid, DATA['Pyramid']?.name || 'Protege', c, s)
    },
    {
        name: 'Torus',
        title: DATA['Torus']?.title || 'Machine Learning',
        description: DATA['Torus']?.desc || 'Entrenamos y optimizamos algoritmos.',
        generator: (c, s) => generateShapeWithText(generateTorus, DATA['Torus']?.name || 'Entrena', c, s)
    },
    {
        name: 'Galaxy',
        title: DATA['Galaxy']?.title || 'Interconectado',
        description: DATA['Galaxy']?.desc || 'Desarrollo de estrategias digitales.',
        generator: (c, s) => generateShapeWithText(generateGalaxy, DATA['Galaxy']?.name || 'Conecta', c, s)
    },
    {
        name: 'Wave',
        title: DATA['Wave']?.title || 'Y visualizado',
        description: DATA['Wave']?.desc || 'Mantén el control, automatiza procesos.',
        generator: (c, s) => generateShapeWithText(generateWave, DATA['Wave']?.name || 'Visualiza', c, s)
    }
];
...
export function generateSphere(count, size) {
    ...
    const shapeCount = Math.floor(count * 0.7);
    const letterCount = count - shapeCount;
    ...
    for (let i = 0; i < letterCount; i++) {
        ...
        if (letterChoice < 0.4) {
            x = -0.18 + (Math.random() - 0.5) * 0.09;
            y = 0.15 + Math.random() * 0.25;
            z = (Math.random() - 0.5) * 0.1;
        } else if (letterChoice < 0.45) {
            x = -0.18 + (Math.random() - 0.5) * 0.1;
            y = 0.5 + (Math.random() - 0.5) * 0.07;
            z = (Math.random() - 0.5) * 0.1;
        } else if (letterChoice < 0.75) {
            x = 0.08 + (Math.random() - 0.5) * 0.05;
            y = 0.15 + Math.random() * 0.35;
            z = (Math.random() - 0.5) * 0.1;
        } else {
            x = -0.08 + Math.random() * 0.28;
            y = 0.4 + (Math.random() - 0.5) * 0.05;
            z = (Math.random() - 0.5) * 0.1;
        }

        points[idx * 3] = x * size;
        points[idx * 3 + 1] = (y + itOffsetY) * size;
        points[idx * 3 + 2] = z * size;
        idx++;
    }
...
export function generate404(count, size) {
    ...
    const points2D = generateTextPoints(text, count, 350, 'Arial', -18);
    const points = new Float32Array(count * 3);
