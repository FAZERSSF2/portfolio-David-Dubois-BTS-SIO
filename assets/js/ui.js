import anime from 'anime.js'; import { switchToScrollMode, switchTo3DMode } from './scroll.js'; export class UI {
        ...
        this.navLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                const href = link.getAttribute('href'); if (href === '#') { e.preventDefault(); switchTo3DMode() } else if (href.startsWith('#') && href.length > 1) {
                    e.preventDefault(); switchToScrollMode(); const targetId = href.substring(1); let targetElement = document.getElementById(targetId); if (!targetElement) { const reverseMap = { 'servicios': 'services', 'clientes': 'clients', 'contacto': 'contact', 'projets': 'projets' }; if (reverseMap[targetId]) { targetElement = document.querySelector(`div[data-dynamic-section="${reverseMap[targetId]}"]`) } }
                    if (targetElement) { targetElement.scrollIntoView({ behavior: 'smooth' }) }
                }
            })
        }); const observerOptions = { root: null, threshold: 0.2 }; const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    if (window.scrollY < 50) return; let id = entry.target.getAttribute('id'); if (!id && entry.target.hasAttribute('data-dynamic-section')) { const sectionName = entry.target.getAttribute('data-dynamic-section'); const map = { 'services': 'servicios', 'clients': 'clientes', 'contact': 'contacto', 'projets': 'projets' }; id = map[sectionName] }
                    if (id) { this.updateActiveNavLink(id) }
                }
            })
        }, observerOptions); this.sections.forEach(section => observer.observe(section))
    }
    updateActiveNavLink(id) { this.navLinks.forEach(link => { const href = link.getAttribute('href'); if (href === '#' + id || (id === '' && href === '#')) { link.classList.add('active') } else { link.classList.remove('active') } }); if (id === 'servicios') { document.body.classList.add('view-services') } else { document.body.classList.remove('view-services') } }
}
