import { SHAPES } from './shapes.js'; let activeShapes = SHAPES; import *as THREE from 'three'; import { OrbitControls } from '/portfolio-David-Dubois-BTS-SIO/assets/dependencies/OrbitControls.js'; import { EffectComposer } from '/portfolio-David-Dubois-BTS-SIO/assets/dependencies/EffectComposer.js'; import { RenderPass } from '/portfolio-David-Dubois-BTS-SIO/assets/dependencies/RenderPass.js'; import { UnrealBloomPass } from '/portfolio-David-Dubois-BTS-SIO/assets/dependencies/UnrealBloomPass.js'; import anime from 'animejs'; import { createNoise3D, createNoise4D } from 'simplex-noise'; let scene, camera, renderer, controls, clock; let composer, bloomPass; let particlesGeometry, particlesMaterial, particleSystem; let currentPositions, sourcePositions, targetPositions, swarmPositions; let particleSizes, particleOpacities, particleEffectStrengths; let noise3D, noise4D; let morphTimeline = null; let isInitialized = !1; let isMorphing = !1; let isInteractionActive = !1; let isSettled = !0; let interactionTimer = null; const raycaster = new THREE.Raycaster(); const mouse = new THREE.Vector2(-9999, -9999); const interactionPlane = new THREE.Plane(new THREE.Vector3(0, 0, 1), 0); const rayLine = new THREE.Line3(); const getOptimizedParticleCount = () => { const isMobile = window.innerWidth < 768; const hardwareConcurrency = navigator.hardwareConcurrency || 3; if (isMobile) { return hardwareConcurrency <= 3 ? 4500 : 6200 } else { return hardwareConcurrency <= 3 ? 11000 : 15000 } }; const optimizedParticleCount = getOptimizedParticleCount(); const optimizedStarCount = Math.floor(optimizedParticleCount * 1.2); const CONFIG = { particleCount: optimizedParticleCount, shapeSize: 14, swarmDistanceFactor: 2.5, swirlFactor: 4.0, noiseFrequency: 0.1, noiseTimeScale: 0.04, noiseMaxStrength: 2.8, colorScheme: 'neon', morphDuration: 4000, particleSizeRange: [0.08, 0.25], starCount: optimizedStarCount, bloomStrength: 1.3, bloomRadius: 0.5, bloomThreshold: 0.05, idleFlowStrength: 0.25, idleFlowSpeed: 0.08, idleRotationSpeed: 0.02, morphSizeFactor: 0.5, morphBrightnessFactor: 0.6, interactionRadius: 2.5, interactionStrength: 3.0 }; let currentShapeIndex = 0; const morphState = { progress: 0.0 }; let activeColorScheme; const COLOR_SCHEMES = { fire: { startHue: 0, endHue: 45, saturation: 0.95, lightness: 0.6 }, neon: { startHue: 300, endHue: 180, saturation: 1.0, lightness: 0.65 }, nature: { startHue: 90, endHue: 160, saturation: 0.85, lightness: 0.55 }, rainbow: { startHue: 0, endHue: 360, saturation: 0.9, lightness: 0.6 } }; const tempVec = new THREE.Vector3(); const sourceVec = new THREE.Vector3(); const targetVec = new THREE.Vector3(); const swarmVec = new THREE.Vector3(); const noiseOffset = new THREE.Vector3(); const flowVec = new THREE.Vector3(); const bezPos = new THREE.Vector3(); const swirlAxis = new THREE.Vector3(); const currentVec = new THREE.Vector3(); const repelVec = new THREE.Vector3(); const closestPoint = new THREE.Vector3(); function init(options = {}) {
...
function createStarfield() {
    const starVertices = []; const starSizes = []; const starColors = []; const starGeometry = new THREE.BufferGeometry(); for (let i = 0; i < CONFIG.starCount; i++) {
        tempVec.set(THREE.MathUtils.randFloatSpread(400), THREE.MathUtils.randFloatSpread(400), THREE.MathUtils.randFloatSpread(400)); if (tempVec.length() < 100) tempVec.setLength(100 + Math.random() * 300); starVertices.push(tempVec.x, tempVec.y, tempVec.z); starSizes.push(Math.random() * 0.15 + 0.05); const color = new THREE.Color(); if (Math.random() < 0.1) { color.setHSL(Math.random(), 0.7, 0.65) } else { color.setHSL(0.6, Math.random() * 0.1, 0.8 + Math.random() * 0.2) }
        starColors.push(color.r, color.g, color.b)
    }
    starGeometry.setAttribute('position', new THREE.Float32BufferAttribute(starVertices, 3)); starGeometry.setAttribute('color', new THREE.Float32BufferAttribute(starColors, 3)); starGeometry.setAttribute('size', new THREE.Float32BufferAttribute(starSizes, 1)); const starMaterial = new THREE.ShaderMaterial({
        uniforms: { pointTexture: { value: createStarTexture() } }, vertexShader: `
...
function setupParticleSystem() {
    ...
    }); particleSystem = new THREE.Points(particlesGeometry, particlesMaterial); particleSystem.position.x = 12; scene.add(particleSystem)
}
...
let isCanvasVisible = !0;
function animate() {
    requestAnimationFrame(animate); if (!isInitialized || !isCanvasVisible || document.hidden) return; const elapsedTime = clock.getElapsedTime(); const deltaTime = clock.getDelta(); controls.update(); const positions = particlesGeometry.attributes.position.array; const effectStrengths = particlesGeometry.attributes.aEffectStrength.array; if (isMorphing) { updateMorphAnimation(positions, effectStrengths, elapsedTime, deltaTime) } else { updateIdleAnimation(positions, effectStrengths, elapsedTime, deltaTime) }
...
function setupCanvasVisibilityObserver(canvas) {
    const obs = new IntersectionObserver((entries) => { isCanvasVisible = entries[0].isIntersecting; }, { threshold: 0 });
    obs.observe(canvas);
}
